...
function edit(id){
	location.href="./roleEdit?id="+id;
}
function delRole(id,roleName){
	if(confirm("确定删除 ["+roleName+"] 角色吗？")){
		location.href="./delRole?id="+id;
	}
}

function roleResourceSet(roleId){
	location.href="./roleResourceSet?roleId="+roleId;
}
</script>
...
<div class="wrapper">
    ...
    <div class="content">
        ...
        <div class="widget">
            <div class="head">
            	<h5 class="iFrames">角色列表</h5>
            	<div class="num"><a href="javascript:;" onclick="add();" class="redNum">&nbsp;新增角色&nbsp;</a></div>
            </div>
            <table cellpadding="0" cellspacing="0" border="0" class="display" id="example">
                ...
                <tbody>
                	...
                	<c:forEach items="${dataList}"  var="dataItem" varStatus="rowStatus" >
                   	 <tr class="gradeA">
						<td class="center">${dataItem.id}</td>
						<td class="center">${dataItem.roleName}</td>
						<td class="center">
						<c:choose>
						...
						<td class="center" title='<fmt:formatDate value="${dataItem.createTime}" pattern="yyyy-MM-dd HH:mm:ss"/>'>	
                           <fmt:formatDate value="${dataItem.createTime}" pattern="yyyy-MM-dd"/>
						</td>
						<td class="center" title='<fmt:formatDate value="${dataItem.lastUpdate}" pattern="yyyy-MM-dd HH:mm:ss"/>'>
						...
						<td class="center">
							<a href="javascript:;" onclick="edit(${dataItem.id});">编辑</a>&nbsp;|
							 ...
							 <c:choose>
								<c:when test="${dataItem.status==0}">
									 <a href="javascript:;" onclick="changeRoleStatus(${dataItem.id},1);">启用</a>&nbsp;|
								</c:when>					
								<c:otherwise>
		                            <a href="javascript:;" onclick="changeRoleStatus(${dataItem.id},0);">禁用</a>&nbsp;|
								</c:otherwise>
							</c:choose>
							   -->         
							 <a href="javascript:;" onclick="roleResourceSet(${dataItem.id});">权限</a>&nbsp;|
							 <a href="javascript:;" onclick="delRole(${dataItem.id},'${dataItem.roleName}');">删除</a>
						</td>
