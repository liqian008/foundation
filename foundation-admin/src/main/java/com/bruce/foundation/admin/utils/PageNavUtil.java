...
public class PageNavUtil {
	...
	public static String getPageNavHtml(int currentPage, int pageSize,
		...
		pageNavHtml.append("<li>共&nbsp;"+totalRows+"&nbsp;条</li>");
		...
		if (totalPage > 1) {
			...
			// 需要显示 首页
//			if (currentPage > 1) {
				pageNavHtml.append("<li class='first'><a href='?page=1'>首页</a></li>");
//			}

			// 如果有上一页
			if (currentPage > beginNum) {
				pageNavHtml.append("<li><a href='?page=" + (currentPage - 1)+"'>&lt;</a></li>");
			}

			for (int i = beginNum; i <= endNum; i++) {
				if (i == currentPage) {
					pageNavHtml.append("<li><a href='?page="+i+"' class='active'>"+i+"</a></li>");
				} else {
					pageNavHtml.append("<li><a href='?page="+i+"'>"+i+"</a></li>");
				}
...
//			if (currentPage < totalPage) {
				pageNavHtml.append("<li class='first'><a href='?page="+totalPage+"'>尾页</a></li>");
//			}
		}
